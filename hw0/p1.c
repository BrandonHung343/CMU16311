...
void DotVectors ( vector* pA, vector* pB, vector* pResult)
{
    if (pA == NULL || pB == NULL || pResult == NULL)
        return;
    float dot = pA->v[0] * pB->v[0] + pA->v[1] * pB->v[1] + pA->v[2] * pB->v[2];
    pResult->v[0] = dot;
    pResult->v[1] = 0.0;
    pResult->v[2] = 0.0;
}
...
void CrossVectors ( vector* pA, vector* pB, vector* pResult)
{
    vector* v1 = pA;
    vector* v2 = pB;
    pResult->v[0] = v1->v[1] * v2->v[1] - v1->v[2] * v2->v[2];
    pResult->v[1] = - (v1->v[0] * v2->v[0] - v1->v[2] * v2->v[2]);
    pResult->v[2] = v1->v[0] * v2->v[0] - v1->v[1] * v2->v[1]; 
}
...
void MultMatrices ( matrix* pA, matrix* pB, matrix* pResult)
{
    ...
    int j;
    for (i = 0; i < 3; i++) { 
        float row[3] = {pA->m[i][0], pA->m[i][1], pA->m[i][2]};
        ...
        // PrintVector(v1);
        for (j = 0; j < 3; j++) {
            float col[3] = {pB->m[0][j], pB->m[1][j], pB->m[2][j]};
            vector* v2 = malloc(sizeof(vector));
            v2->v[0] = col[0];
            v2->v[1] = col[1];
            v2->v[2] = col[2];
            vector* vRes = malloc(sizeof(vector));
            // PrintVector(v2);
            DotVectors(v1, v2, vRes);
            // printf("%.3f\n", vRes->v[0]);
            pResult->m[i][j] = vRes->v[0];
            free(vRes);
            free(v2);
        }
        free(v1);
...
void MultMatVec ( vector* pV, matrix* pM, vector* pResult)
{
    int j;
    for (j = 0; j < 3; j++) {
        float col[3] = {pM->m[0][j], pM->m[1][j], pM->m[2][j]};
        vector* v2 = malloc(sizeof(vector));
        v2->v[0] = col[0];
        v2->v[1] = col[1];
        v2->v[2] = col[2];
        vector* vRes = malloc(sizeof(vector));
        // PrintVector(v2);
        DotVectors(pV, v2, vRes);
        // printf("%.3f\n", vRes->v[0]);
        pResult->v[j] = vRes->v[0];
        free(vRes);
        free(v2);
    }
    // PrintVector(pResult);
